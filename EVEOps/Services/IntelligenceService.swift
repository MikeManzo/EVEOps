...
actor IntelligenceService {
    ...
    // MARK: Cross-Domain Briefing History
    //
    // A read-only, passive index over insights already generated by the per-domain
    // analyze* calls above. It records nothing new on its own — every entry is a
    // byproduct of the user having visited Finances, Skill Planner, Killmails,
    // Industry, Assets, or Clones/Implants this session. The Dashboard's "Daily
    // Briefing" widget reads this to surface a few AI findings alongside its own
    // deterministic checks, without triggering any additional model generation.

    enum BriefingDomain: String, Sendable {
        case finance, skills, combat, industry, assets, implants

        var label: String {
            switch self {
            case .finance:   return "Finances"
            case .skills:    return "Skill Training"
            case .combat:    return "Combat"
            case .industry:  return "Industry"
            case .assets:    return "Assets"
            case .implants:  return "Clones & Implants"
            }
        }
        ...
        var icon: String {
            ...
            case .implants:  return "brain.head.profile"
            }
        }
    }

    struct BriefingEntry: Sendable, Identifiable {
        let id = UUID()
    ...
    private var briefingEntries: [String: BriefingEntry] = [:]

    private func recordBriefing(_ domain: BriefingDomain, characterName: String, headline: String, suggestion: String) {
        guard !characterName.isEmpty else { return }
        let key = "\(domain.rawValue)|\(characterName)"
        briefingEntries[key] = BriefingEntry(
            domain: domain, characterName: characterName,
            headline: headline, suggestion: suggestion, generatedAt: Date()
        )
    }

    /// The most recently generated insights across every domain and character analyzed
    /// this session, newest first. Purely an aggregation of existing caches — never
    /// triggers new generation itself.
    func recentBriefingEntries(limit: Int = 6) -> [BriefingEntry] {
        Array(briefingEntries.values.sorted { $0.generatedAt > $1.generatedAt }.prefix(limit))
    }
 ...
 //           log.debug("Raw model output: \(raw)")
    ...
    func analyzeFinances(
        ...
        recordBriefing(.finance, characterName: characterName, headline: result.summary, suggestion: result.suggestion)
        return result
    }
    ...
    func analyzeTrainedSkills(
        ...
        if let top = result.recommendations.first {
            recordBriefing(
                .skills, characterName: characterName,
                headline: result.playstyleSummary,
                suggestion: "Train \(top.skillName) to L\(top.targetLevel) — \(top.rationale)"
            )
        }
        return result
    }
    ...
    func analyzeCombat(
        ...
        recordBriefing(.combat, characterName: characterName, headline: result.summary, suggestion: result.suggestion)
        return result
    }
    ...
    func analyzeIndustry(
        ...
        recordBriefing(.industry, characterName: characterName, headline: result.summary, suggestion: result.suggestion)
        return result
    }
    ...
    func analyzeAssets(
        ...
        recordBriefing(.assets, characterName: characterName, headline: result.summary, suggestion: result.suggestion)
        return result
    }
    ...
    func analyzeImplants(
        ...
        recordBriefing(.implants, characterName: characterName, headline: result.setAssessment, suggestion: result.recommendation)
        return result
    }
