...
struct AssetAIInsightCard: View {
    ...
    var body: some View {
        if aiInsightsEnabled && aiInsightAssets, case .available = model.availability {
            AIInsightCard(
                loadingMessage: "Analyzing asset spread\u{2026}",
                taskID: assets.first?.itemId ?? 0,
                generate: generate
            ) { (insight: AssetInsight) in
                StandardInsightBody(summary: insight.summary, suggestion: insight.suggestion)
            }
        }
    }

    private func generate() async throws -> AssetInsight {
        ...
        return try await IntelligenceService.shared.analyzeAssets(
