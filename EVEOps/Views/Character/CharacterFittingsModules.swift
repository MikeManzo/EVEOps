...
struct FittingAIInsightCard: View {
    ...
    var body: some View {
        if aiInsightsEnabled && featureEnabled, case .available = model.availability {
            AIInsightCard(
                loadingMessage: "Analyzing fitting\u{2026}",
                taskID: shipName,
                generate: generate
            ) { (insight: FittingInsight) in
                StandardInsightBody(summary: insight.roleAssessment, suggestion: insight.suggestion)
            }
        }
    }

    private func generate() async throws -> FittingInsight {
        try await IntelligenceService.shared.analyzeFitting(
            shipName: shipName,
            shipClass: shipClass,
