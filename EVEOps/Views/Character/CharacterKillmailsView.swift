...
struct CombatAIInsightCard: View {
    ...
    var body: some View {
        if aiInsightsEnabled && aiInsightKillmails, case .available = model.availability {
            AIInsightCard(
                loadingMessage: "Analyzing combat record\u{2026}",
                taskID: groups.first?.characterID ?? 0,
                generate: generate
            ) { (insight: CombatInsight) in
                StandardInsightBody(summary: insight.summary, suggestion: insight.suggestion)
            }
        }
    }

    private func generate() async throws -> CombatInsight {
        ...
        return try await IntelligenceService.shared.analyzeCombat(
