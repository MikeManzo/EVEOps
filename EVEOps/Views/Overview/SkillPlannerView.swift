    ...
    private var planPanel: some View {
        VStack(spacing: 0) {
            ...
            if #available(macOS 26.0, *), let info = selectedCharInfo {
                SkillPlanAIInsightCard(characterInfo: info)
...
struct SkillPlanAIInsightCard: View {
    ...
    private func generate() async {
        ...
        generationError = nil
        do {
            let topGroups: [(name: String, spFormatted: String, skillCount: Int, maxedCount: Int)] = characterInfo.skillGroups
                .map { group in
            ...
            let notableSkills: [String] = characterInfo.skillGroups
                .flatMap { $0.skills }
                .filter { $0.trainedLevel >= 4 }
                .sorted { $0.skillpoints > $1.skillpoints }
                .prefix(12)
                .map { "\($0.name) L\($0.trainedLevel)" }

            recommendation = try await IntelligenceService.shared.analyzeTrainedSkills(
