...
struct IndustryAIInsightCard: View {
    ...
    var body: some View {
        if aiInsightsEnabled && aiInsightIndustry, case .available = model.availability {
            AIInsightCard(
                loadingMessage: "Analyzing industry data\u{2026}",
                taskID: jobs.first?.characterName ?? "",
                generate: generate
            ) { (insight: IndustryInsight) in
                StandardInsightBody(summary: insight.summary, suggestion: insight.suggestion)
            }
        }
    }

    private func generate() async throws -> IndustryInsight {
        ...
        return try await IntelligenceService.shared.analyzeIndustry(
